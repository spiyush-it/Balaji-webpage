...
(function () {
  ...
  /* ── Mobile detection: skip zoom entirely on touch/mobile devices ──
     Uses three independent signals — any one match = mobile:
     1. UA string covers Android, iPhone, iPad, etc.
     2. Touch support with at least 1 touch point (catches Android)
     3. Screen width ≤ 1024px with any touch support (tablets/phones)
  ── */
  function isMobile() {
    var ua = navigator.userAgent || '';
    var uaMatch = /Mobi|Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini|Mobile/i.test(ua);
    var hasTouch = ('ontouchstart' in window) || (navigator.maxTouchPoints >= 1);
    var narrowScreen = window.screen.width <= 1024;
    return uaMatch || (hasTouch && narrowScreen);
  }
